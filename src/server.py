...
class WebSite:
    def __init__ (self, fp):
        ...
        for x in self.config.parse_server ():
            if x.ssl != False:
                self.servers[x.port] = WebServerSSL ((socket.gethostbyname(socket.gethostname()), x.port), handler.WebHandler, x.ssl)
            else:
                self.servers[x.port] = WebServer ((socket.gethostbyname(socket.gethostname()), x.port), handler.WebHandler)
